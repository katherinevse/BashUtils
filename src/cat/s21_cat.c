...
void parser(int argc, char *argv[], opt *options) {
  ...
  static struct option long_options[] = {
      {"number-nonblank", 0, 0, 'b'},  // из видео статик
  ...
  while ((flag = getopt_long(argc, argv, "+benstvTE", long_options, 0)) !=
         -1) {  // тут почему 0 в конце и почему -1 // + если мы дойдем до флага
                // -> все остальное это файлы
    switch (flag) {
      case 'b':
        options->b = 1;
        break;
    ...
    while ((cur = fgetc(file)) != EOF) {  // считываем из файла
      ...
      if (options->v) {
        if ((cur < 9 || cur > 10) && cur < 32) {
          printf("^");
          cur = cur + 64;  // неотображаемые управляющие символы представляются
                           // в виде ^ и соответствующей буквы.
